...
class Aggregator(job_api_pb2_grpc.JobServiceServicer):
    ...
    def testing_completion_handler(self, client_id, results):
        ...
        if len(self.test_result_accumulator) == len(self.executors):
            ...
            # FIXME self.save_model() disabled because it assumes that self.model_weights is a numpy array where it is in fact a list of torch tensors
            # The following is the output from running configs/femnist/conf.yml
